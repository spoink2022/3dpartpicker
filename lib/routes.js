...
const { getUser, getNearbySellers } = require('./getter/accounts.js');
...
router.get('/print', async (req, res) => {
  let part = await getPart(req.query.product);
  let user = await getUser(req.query.user);
  let avatarUrl = user.avatar_url || 'https://storage.googleapis.com/3dpartpicker-files/users/default.png';
  let fullName = user.first_name + ' ' + user.last_name;
  let sellers = await getNearbySellers(user.lat, user.lon, 10);
  console.log(sellers);
